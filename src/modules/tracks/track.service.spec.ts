...
import { BundleMusic } from './domain/entities/bundle-music.entity';
...
describe('MusicService', () => {
    ...
    let mockRepository: MockProxy<TrackRepository>;
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({
            ...
            .overrideProvider(TrackRepository)
            .useValue(mock<TrackRepository>())
            .compile();

        service = module.get<TrackService>(TrackService);
        mockRepository = module.get(TrackRepository);
    });

    describe('createBundleMusic', () => {
        it('기존에 정보가 존재하는 음악을 꾸러미에 생성 한다.', async () => {
            const createMusicDto: CreateTrackDto = {
